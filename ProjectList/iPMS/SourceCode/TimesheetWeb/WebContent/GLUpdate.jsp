...
<%  UserInfoBean beanUserInfo = (UserInfoBean)session.getAttribute("beanUserInfo");
    GLUpdateBean beanGLUpdate = (GLUpdateBean)request.getAttribute("beanGLUpdate");
    MappingDetailBean beanMappingDetail = (MappingDetailBean)request.getAttribute("beanMappingDetail");
%>
<HTML>
<HEAD>
<TITLE>Update Timesheet By Group Leader</TITLE>
...
<H1><IMG align="top" src="image/tit_UpdateTimesheetByOther.gif"></H1>

<FORM method="post" action="TimesheetServlet" name="frmGLUpdate" onreset="javascript:resetUpdate();">
<INPUT type="hidden" name="hidAction" value="">
<INPUT type="hidden" name="hidActionDetail" value="">
<INPUT type="hidden" name="Role" value="<%=beanUserInfo.getRole()%>">
<INPUT type="hidden" name="maxRow" value="<%=beanGLUpdate.getTimesheetList().getNumberOfRows()%>">
...
<TABLE border="0" cellpadding="3" cellspacing="0" width="100%" bgcolor="#336699">
    ...
    int nRows=0;
    try {
        StringMatrix mtxTimesheet = beanGLUpdate.getTimesheetList();
        StringMatrix mtxProcess = beanGLUpdate.getProcessList();
        int processRows = mtxProcess.getNumberOfRows();
        StringMatrix mtxType = beanGLUpdate.getTypeList();
        int typeRows = mtxType.getNumberOfRows();
        StringMatrix mtxProduct = beanGLUpdate.getProductList();
 ...
 %>
    ...
    <TR>
        ...
        <!-- Must update -->
        <!-- sProcess -->
        <TD width="15%" class="<%=strClass%>"><SELECT size="1" name="Process" class="SmallCombo" onchange="javascript:selectProcess(<%=i%>, this.value, <%=sProductId%>);"><%
            j = 0;
            while (j < processRows) {
...
%>
        ...
        <TD width="15%" align="center" class="<%=strClass%>">
...
function doUpdateAndApprove() {
    ...
    form.hidActionDetail.value = "SaveUpdateAndApproveGL";
