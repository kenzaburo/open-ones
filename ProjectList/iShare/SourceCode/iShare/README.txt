This project is developed from jForum (jforum.net) 2.1.8.

How to run the project from Eclipse
---------------------------------------------------------------------------------
Prepare the MySQL Database
1) Run SQL client with root permission
mysql -h localhost -u root -p

2) Execute scripts in dbscript\CreateDB.sql to create database and user
Database name: jforum
Account: adminforum/adminforum

3) Prepare IDE environment: Eclipse + Tomcat 6
For the first time, right-click on /jForum/WebContent/install.jsp to run On Server

__________________
Open-Ones Group (http://open-ones.com)
thachln@gmail.com
